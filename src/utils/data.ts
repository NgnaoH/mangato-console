...
export const getUserRole = (user: AdditionalUser): string => {
  if (user.authRole === userRole.admin) return userRole.admin;
  if (user.authRole === userRole.user && user.isVerified)
    return userRole.uploader;
  if (user.authRole === userRole.user && !user.isVerified) return userRole.user;
};
