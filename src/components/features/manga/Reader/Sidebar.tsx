...
import { LANGUAGES_TRANSLATE } from "@/constants/vi";
      ...
      <div className="flex flex-col h-full w-full p-4 space-y-2">
        ...
        <div className="flex items-center justify-center space-x-2">
          ...
          <ReportConfirmation
            reference={
              <ButtonTooltip
                tooltip={<p>Report chapter</p>}
                LeftIcon={MdReport}
              />
            }
            type={typeReport.chapter}
            onConfirm={(reason) =>
              handleReportChapter(reason, currentChapter.slug)
            }
          />
        </div>
        {currentChapter.multilang === multilangStatusCons.ready && (
          <div className="flex items-center justify-center">
            <Select
              options={LANGUAGES_TRANSLATE}
              value={{
                value: lang,
                label: LANGUAGES_TRANSLATE.find((e) => e.value === lang)?.label,
              }}
              isClearable={false}
              onChange={({ value }) => {
                setLang(value);
                if (value === currentChapter.language) return;
                if (currentChapter.multilang !== multilangStatusCons.ready) return;
                getTranslate(value, images);
              }}
            />
          </div>
        )}
          ...
          <ul className="h-full overflow-auto bg-background-900">
            {filteredChapters.map((chapter) => {
              ...
              return (
                <li
                  className={classNames(
                    "relative px-4 py-2 cursor-pointer hover:bg-white/20 transition duration-300 flex justify-between",
