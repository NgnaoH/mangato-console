...
import { getImagesTranslate } from "@/services/upload";
...
const ReadPage: NextPage<ReadPageProps> = ({ chapters }) => {
  ...
  const [isTranlating, setIsTranlating] = useState(false);
  const [imagesTranslated, setImagesTranslated] = useState([]);
  ...
  useEffect(() => {
    setCurrentLang(currentChapter.language);
  }, [currentChapter]);

  useEffect(() => {
    if (isUndefined(data?.imagesId) || currentChapter.multilang !== multilangStatusCons.ready) return;
    supabase
      .from("images_detected")
      .select("additionData")
      .eq("imagesId", data.imagesId)
      .single()
      .then((res) => setImagesBoxes(res?.data?.additionData));
  }, [data, currentChapter]);
  ...
  const getTranslate = async (lang: string, images: ImageSource[]) => {
    setImagesTranslated([]);
    setIsTranlating(true);

    const uploadedImages = await getImagesTranslate({
      images: images.map((e) => ({ boxes: e.boxes })),
      sourceLanguageCode: currentChapter.language,
      targetLanguageCode: lang,
    });

    setImagesTranslated(uploadedImages);
    setIsTranlating(false);
  };

  const imagesNomallize = (
    images: ImageSource[],
    imagesBoxes: any,
    multilang: string
  ) => {
    if (isEmpty(images) || !imagesBoxes.length || multilang !== multilangStatusCons.ready) return images;
    return images.map((image) => ({
      ...image,
      boxes: imagesBoxes.find((e) => image.url === e.url)?.boxes,
    }));
  };
  ...
  return (
    <ReadContextProvider
      value={{
        ...
        sourceId,
        images: imagesNomallize(
          data?.images,
          imagesBoxes,
          currentChapter.multilang
        ),
        imagesId: data?.imagesId,
        isTranslate: currentChapter.language !== currentLang,
...
export const getServerSideProps: GetServerSideProps = async ({
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select(`chapters:kaguya_chapters(*, source:kaguya_sources(*))`)
