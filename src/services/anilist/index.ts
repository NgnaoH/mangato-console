...
const anilistFetcher = async <T>(query: string, variables: any) => {
  type Response = {
    data: T;
  };

  try {
    const { data } = await axios.post<Response>(config.anilistUrl, {
      query,
      variables,
    });
    return data?.data;
...
export const getMedia = async (args: MediaArgs & PageArgs, fields?: string) => {
  ...
  // @ts-ignore
  const { data: mediaTranslations, error } = await supabase
