...
export const upsertChapter = async (args: UpsertChapterArgs) => {
  const { sourceId, chapter, mediaId, lang } = args;
...
export const getImagesTranslate = async ({
  images,
  sourceLanguageCode,
  targetLanguageCode,
}: any) => {
  const { data } = await client.post<any>("/translate", {
    sourceLanguageCode,
    targetLanguageCode,
    images,
  });
...
export const updateUserPermission = async (body: any) => {
  const { data } = await client.put<any>("/user", body);
